...
var agentdb = {
    ...
    getSource : function (x) {
        ...
        if (sourceId >= this.database.sources.length) {
            if (debug) { console.error("getSource: x=" + x + ", sourceId=" + sourceId); }
            return "Unknown";
        }
    ...
    lookupHash : function(agent) {
        ...
        if (typeof agent != 'string') {
            if (debug) { console.error("samplx-agentdb::lookupHash: non-string argument."); }
            return null;
        }
        var r = this.database.hash.lookup(agent, 'a');
        if (r === null) {
            if (debug) { console.error("samplx-agentdb::lookupHash: not found."); }
            return null;
        }
    ...
    lookupPattern : function(agent) {
        ...
        if (typeof agent != 'string') {
            if (debug) { console.error("samplx-agentdb::lookupPattern: non-string argument."); }
            return null;
        }
        ...
        if (debug) { console.error("samplx-agentdb::lookupPattern: not found."); }
        return null;
    },
    
    lookup : function(agent) {
        if (debug) { console.error("samplx-agentdb::lookup('" + agent + "')"); }
        ...
        if (debug) { console.error("samplx-agentdb::lookup: found result=" + util.inspect(r)); }
        return r;
    },
    ...
    sendFeedback : function(agents, reportingUrl) {
        if (util.isArray(agents) && (agents.length > 0)) {
            ...
            var req = http.request(options, function(res) {
                if (debug) { console.error('samplx-agentdb::sendFeedback'); }
                if (debug) { console.error('STATUS: ' + res.statusCode); }
                if (debug) { console.error('HEADERS: ' + JSON.stringify(res.headers)); }
                res.setEncoding('utf8');
                res.on('data', function(chunk) {
                    if (debug) { console.error('samplx-agentdb::sendFeedback: BODY: ' + chunk); }
                });
                
                res.on('end', function () {
                    if (debug) { console.error('samplx-agentdb::sendFeedback: end of request.'); }
                });
            });

            req.on('error', function(e) {
                if (debug) { console.error('samplx-agentdb::sendFeedback: Error on request: ' + e.message); }
                throw e;
            });
            req.write(postData);
            req.end();
